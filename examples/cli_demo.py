    ...
    if args.stdout_logging is not None:
        if args.stdout_logging:
            os.environ["PARALLEL_STDOUT_LOGGING"] = (
                args.log_level.upper() if args.log_level else "INFO"
            )
        else:
...
Examples:
  ...
  %(prog)s --show-config-only
    ...
    parser.add_argument(
        "--empty-log-file", action="store_true", help="Empty log file on startup"
    )
    parser.add_argument(
        "--no-empty-log-file",
        dest="empty_log_file",
    ...
    parser.add_argument(
        "--stdout-logging", action="store_true", help="Enable stdout logging"
    )
    parser.add_argument(
        "--no-stdout-logging",
        dest="stdout_logging",
    ...
    parser.add_argument(
        "--analyze-only", action="store_true", help="Only analyze existing log file"
    )
