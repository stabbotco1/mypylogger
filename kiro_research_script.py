    ...
    for url in sites_to_check:
        try:
            ...
            results.append({
                ...
                'title_found': '<title>' in response.text.lower() if response.status_code == 200 else False
            })
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            results.append({
                ...
                'description': data.get('info', {}).get('summary', 'No description')
            })
        else:
            results.append({
                'package_manager': 'PyPI',
...
def main():
    ...
    # Analyze results
    accessible_sites = [site for site in website_results if site.get('accessible', False)]
    ...
    research_data['summary'] = {
        ...
        'successful_searches': len([s for s in research_data['searches_performed'] if s.get('found_content', False)])
    }
    
    # Generate recommendations
    if accessible_sites:
        research_data['recommendations'].append(f"Found {len(accessible_sites)} accessible Kiro-related websites")
    
    if found_packages:
        research_data['recommendations'].append(f"Found {len(found_packages)} Kiro packages in package managers")
    
    if not accessible_sites and not found_packages:
        research_data['recommendations'].append("No direct Kiro IDE resources found - may need alternative research methods")
    ...
    print(f"  - Successful searches: {research_data['summary']['successful_searches']}/{research_data['summary']['total_searches']}")
