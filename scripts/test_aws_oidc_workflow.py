...
def test_workflow_yaml_syntax() -> bool:
    ...
    try:
        ...
        with open(".github/workflows/pypi-publish.yml", "r") as f:
        ...
        for region, description in scenarios:
            ...
            if result.returncode == 0:
                ...
                if expected_region in result.stdout:
                    ...
                    log_test_step(f"Region scenario: {description}", "FAIL", 
...
def test_error_messages() -> bool:
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            missing_phrases = [phrase for phrase in required_phrases 
                             if phrase not in error_output]
            
            if not missing_phrases:
                log_test_step("Missing role ARN error message", "PASS")
            else:
                log_test_step("Missing role ARN error message", "FAIL", 
                            f"Missing phrases: {missing_phrases}")
                return False
        else:
            log_test_step("Missing role ARN error message", "FAIL", 
                        "Should have failed with missing role ARN")
            return False
        ...
        if result.returncode != 0 and "Invalid AWS region format" in result.stderr:
            ...
            log_test_step("Invalid region error message", "FAIL", 
                        "Should provide clear region format error")
            return False
...
def generate_test_report(results: Dict[str, bool]) -> None:
    ...
    if failed_tests > 0:
        print(f"\n🔧 Troubleshooting:")
        print(f"  - Review failed test details above")
        print(f"  - Check AWS configuration validation script")
        print(f"  - Verify workflow YAML syntax")
        print(f"  - Test error message clarity")
    else:
