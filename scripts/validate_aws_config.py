...
def validate_aws_configuration() -> dict:
    ...
    try:
        ...
        aws_secret_name = os.environ.get('AWS_SECRET_NAME') or os.environ.get('INPUT_AWS_SECRET_NAME')
...
def main() -> int:
    ...
    try:
        ...
        print("  2. Verify AWS_REGION is a valid AWS region (or omit for us-east-1 default)", file=sys.stderr)
        print("  3. Check AWS_SECRET_NAME contains the correct Secrets Manager secret name", file=sys.stderr)
        print("  4. Verify the IAM role has proper permissions for Secrets Manager access", file=sys.stderr)
        return 1
