...
def run_command(command: List[str], timeout: int = 60) -> subprocess.CompletedProcess:
    ...
    try:
        ...
        if result.returncode == 0:
            log_structured("INFO", "Command succeeded", 
                         command=" ".join(command),
                         duration=timeout)
        else:
            log_structured("ERROR", "Command failed",
                         command=" ".join(command),
        ...
        log_structured("ERROR", "Command timed out",
                     command=" ".join(command),
                     timeout=timeout)
        raise AWSAuthError(f"Command timed out after {timeout} seconds: {' '.join(command)}")
    
    except Exception as e:
        log_structured("ERROR", "Command execution failed",
                     command=" ".join(command),
                     error=str(e))
...
def validate_aws_connectivity() -> None:
    ...
    try:
        ...
        try:
            ...
            log_structured("INFO", "AWS connectivity validated",
                         account=caller_identity.get("Account"),
                         arn=caller_identity.get("Arn"),
                         user_id=caller_identity.get("UserId"))
        except json.JSONDecodeError:
            log_structured("WARN", "Could not parse caller identity response",
                         stdout=result.stdout)
    ...
    try:
        ...
        if result.returncode != 0:
            if "ResourceNotFoundException" in result.stderr:
                ...
                raise AWSAuthError(f"Access denied to secret '{secret_name}'. Check IAM permissions.")
            else:
...
def configure_aws_credentials_with_retry(
    ...
    """
    log_structured("INFO", "Starting AWS OIDC authentication with retry logic",
                 role_arn=role_arn,
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            for key, value in env_vars.items():
                ...
                log_structured("INFO", f"Set environment variable", key=key, value=value if "ARN" not in key else "***")
            ...
            log_structured("ERROR", f"AWS authentication attempt {attempt} failed",
                         attempt=attempt,
                         error=str(e))
            ...
            if attempt < max_attempts:
                ...
                log_structured("INFO", f"Retrying in {backoff_seconds} seconds...",
                             backoff_seconds=backoff_seconds)
                time.sleep(backoff_seconds)
            else:
                log_structured("ERROR", "All AWS authentication attempts failed",
...
def main() -> int:
    ...
    try:
        ...
        log_structured("INFO", "Starting AWS authentication process",
                     aws_region=aws_region,
        ...
        print("  2. Check that the IAM role has proper trust relationship with GitHub OIDC", file=sys.stderr)
