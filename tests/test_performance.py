...
class TestPerformanceBenchmarks:
    ...
    def test_logging_latency_requirement(self, temp_log_dir):
        ...
        assert p95_latency < 1.0, f"95th percentile latency {p95_latency:.3f}ms exceeds 1ms requirement"
        
        # Additional checks for good performance
        assert avg_latency < 0.5, f"Average latency {avg_latency:.3f}ms should be well under 1ms"
    ...
    def test_logging_throughput_requirement(self, temp_log_dir):
        ...
        assert throughput > 10000, f"Throughput {throughput:.0f} logs/sec is below 10,000 requirement"
    ...
    def test_memory_usage_requirement(self, temp_log_dir):
        ...
        for i in range(5000):
            logger.info(f"Memory test message {i} with some additional data to make it realistic")
            ...
            if i % 1000 == 0:
                ...
                assert memory_increase < 50, f"Memory usage increased by {memory_increase:.1f}MB, exceeding 50MB limit"
        ...
        assert total_increase < 50, f"Memory usage increased by {total_increase:.1f}MB, exceeding 50MB limit"
    ...
    def test_concurrent_logging_performance(self, temp_log_dir):
        ...
        assert overall_throughput > 5000, f"Concurrent throughput {overall_throughput:.0f} is too low"
        
        # Ensure no thread performed extremely poorly
        min_thread_throughput = min(result[1] for result in thread_results)
        assert min_thread_throughput > 1000, f"Slowest thread only achieved {min_thread_throughput:.0f} logs/sec"
    ...
    def test_performance_regression_detection(self, temp_log_dir):
        ...
        assert avg_latency_ms < 0.5, f"Average latency {avg_latency_ms:.3f}ms is concerning"
        assert throughput > 5000, f"Throughput {throughput:.0f} logs/sec is below expected baseline"
