...
class TestJSONSchemaValidation:
    ...
    def test_json_output_basic_schema(self, tmp_path, monkeypatch):
        ...
        # Read the log file
        log_file_path = tmp_path / "logs" / f"schema_test_{time.strftime('%Y_%m_%d')}.log"
        ...
        assert isinstance(log_entry['levelname'], str), "levelname field should be string"
    ...
    def test_json_field_order(self, tmp_path, monkeypatch):
        ...
        # Read the log file
        log_file_path = tmp_path / "logs" / f"order_test_{time.strftime('%Y_%m_%d')}.log"
        with open(log_file_path, 'r') as f:
            log_line = f.readline().strip()
        ...
        assert field_order[0] == 'time', f"First field should be 'time', got '{field_order[0]}'"
        
        # Verify expected fields are present in some order
        expected_fields = {'time', 'levelname', 'message', 'filename', 'lineno', 'funcName'}
    ...
    def test_timestamp_format(self, tmp_path, monkeypatch):
        ...
        # Read the log file
        log_file_path = tmp_path / "logs" / f"timestamp_test_{time.strftime('%Y_%m_%d')}.log"
        with open(log_file_path, 'r') as f:
            log_line = f.readline().strip()
        ...
        assert '.' in timestamp, f"Timestamp should contain milliseconds, got: {timestamp}"
        ...
        assert len(milliseconds) == 3, f"Should have 3 millisecond digits, got: {milliseconds}"
        assert milliseconds.isdigit(), f"Milliseconds should be digits, got: {milliseconds}"
    ...
    def test_line_number_as_string(self, tmp_path, monkeypatch):
        ...
        # Read the log file
        log_file_path = tmp_path / "logs" / f"lineno_test_{time.strftime('%Y_%m_%d')}.log"
        with open(log_file_path, 'r') as f:
            log_line = f.readline().strip()
        ...
        assert isinstance(log_entry['lineno'], str), f"lineno should be string, got {type(log_entry['lineno'])}"
        assert log_entry['lineno'].isdigit(), f"lineno should be numeric string, got: {log_entry['lineno']}"
    ...
    def test_null_value_normalization(self, tmp_path, monkeypatch):
        ...
        for key, value in log_entry.items():
            if value is None:
                pytest.fail(f"Found None value in log entry for key '{key}'. Should be excluded or converted to 'null'")
    ...
    def test_unwanted_field_removal(self, tmp_path, monkeypatch):
        ...
        for field in unwanted_fields:
            assert field not in log_entry, f"Unwanted field '{field}' found in log entry"
    ...
    def test_complete_json_schema_compliance(self, tmp_path, monkeypatch):
        ...
        log_file_path = tmp_path / "logs" / f"complete_test_{time.strftime('%Y_%m_%d')}.log"
        with open(log_file_path, 'r') as f:
            log_lines = f.readlines()
        
        assert len(log_lines) == len(test_messages), f"Expected {len(test_messages)} log entries, got {len(log_lines)}"
    ...
    def test_structured_logging_schema(self, tmp_path, monkeypatch):
        ...
        log_file_path = tmp_path / "logs" / f"structured_test_{time.strftime('%Y_%m_%d')}.log"
        with open(log_file_path, 'r') as f:
            log_line = f.readline().strip()
...
class TestJSONOutputVariations:
    ...
    def test_json_with_exception_info(self, tmp_path, monkeypatch):
        ...
        # Read and validate log entry
        log_file_path = tmp_path / "logs" / f"exception_test_{time.strftime('%Y_%m_%d')}.log"
        with open(log_file_path, 'r') as f:
            log_line = f.readline().strip()
    ...
    def test_json_with_unicode_characters(self, tmp_path, monkeypatch):
        ...
        # Read and validate log entry
        log_file_path = tmp_path / "logs" / f"unicode_test_{time.strftime('%Y_%m_%d')}.log"
    ...
    def test_json_with_large_messages(self, tmp_path, monkeypatch):
        ...
        # Read and validate log entry
        log_file_path = tmp_path / "logs" / f"large_test_{time.strftime('%Y_%m_%d')}.log"
        with open(log_file_path, 'r') as f:
            log_line = f.readline().strip()
    ...
    def test_json_consistency_across_multiple_entries(self, tmp_path, monkeypatch):
        ...
        log_file_path = tmp_path / "logs" / f"consistency_test_{time.strftime('%Y_%m_%d')}.log"
        with open(log_file_path, 'r') as f:
            log_lines = f.readlines()
